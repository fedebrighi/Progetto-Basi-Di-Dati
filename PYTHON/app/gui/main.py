...
class CampionatoApp:
    ...
    def create_widgets(self):
        self.label = tk.Label(self.root, text="Benvenuto nel Campionato Manager")
        self.label.grid(column=0, row=0, padx=10, pady=10)
        
        self.btn_iscrizione = tk.Button(self.root, text="Iscrizione Squadra", command=self.open_iscrizione)
        self.btn_iscrizione.grid(column=0, row=1, padx=10, pady=10)
        
        self.btn_scambio = tk.Button(self.root, text="Proporre Scambio Giocatori", command=self.open_scambi)
        self.btn_scambio.grid(column=0, row=2, padx=10, pady=10)
        
        self.btn_infortunio = tk.Button(self.root, text="Registrare Infortunio", command=self.open_infortuni)
        self.btn_infortunio.grid(column=0, row=3, padx=10, pady=10)

        self.btn_visualizza_squadre = tk.Button(self.root, text="Visualizza Squadre", command=self.open_visualizza_squadre)
        self.btn_visualizza_squadre.grid(column=0, row=4, padx=10, pady=10)

        self.btn_visualizza_classifica = tk.Button(self.root, text="Visualizza Classifica", command=self.open_visualizza_classifica)
